...
const Index = () => {
  ...
  // STAGE 1: Center card slides in from LEFT (progress 0.10 → 0.35)
  const centerProgress = clamp01((progress - 0.1) / 0.25);
  const centerEased = ease(centerProgress);
  const centerX = -120 + centerEased * 120; // -120% → 0%
  const centerOpacity = centerEased;

  // STAGE 2: Side cards slide in (progress 0.40 → 0.70)
  const sideProgress = clamp01((progress - 0.4) / 0.3);
  const sideEased = ease(sideProgress);
  const leftX = -140 + sideEased * 140;
  const rightX = 140 - sideEased * 140;
  ...
  return (
    <main className="bg-[#E8ECF5] text-slate-900 overflow-x-hidden font-sans">
      ...
      {/* Built for Growth section — taller to allow staged scroll */}
      <section
        ref={sectionRef}
        className="relative min-h-[200vh] w-full flex items-center justify-center px-6 py-32"
      >
        <div className="sticky top-1/2 -translate-y-1/2 w-full">
          <div className="relative w-full max-w-6xl mx-auto h-[600px] flex items-center justify-center">
            ...
            {/* Center main glass card (STAGE 1 — slides from LEFT) */}
            <div
